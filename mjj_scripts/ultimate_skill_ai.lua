...
--]]
function UltimateSkillAi:buildUltimate(uid)
...
-- @param uids 角色UID集合
--]]
function UltimateSkillAi:handleUltimatePriority(ultimateInfos, uids)
    -- 对大招进行分类
    ...
    if #targets > 0 and  treatment then -- 选择治疗系
        return treatment
    end

    -- 根据BOSS存在与否选择
    if self:haveBoss() == true then
        return decreasePlayerHarm or addPlayerHarm or attackDecreasePlayerHarm or attackAddPlayerHarm or common or {}
    else
        return common or addPlayerHarm or decreasePlayerHarm or {}
    end
end
...
--  大招策略
--]]
function UltimateSkillAi:getUltimateSkillAIBehavior(uid, banAiList)
    -- 初始化数据
    ...
    for _, data in pairs(ultimateData) do
        if data.isCd == true and banTypeMap[data.ultimateType] == nil then -- 不处与CD中, 不被禁用的大招才初始化
            local ultimateInfo = {}
